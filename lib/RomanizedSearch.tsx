...
import { khmerDictionary } from './unique_khmer_datav7';
...
export const searchInDictionary = (inputWord: string) => {
	...
	for (let word in khmerDictionary) {
		...
		if (InputWord === DictionaryWord) {
			const value = khmerDictionary[word];
		...
		//  else if (DictionaryWord.startsWith(InputWord)) {
		// 	const value = khmerDictionary[word];
		// 	if (typeof value === 'string') {
...
export const getClosestMatchesDictionary = (
	...
	for (let romanized in khmerDictionary) {
		...
		if (distance <= threshold) {
			const value = khmerDictionary[romanized];
			if (typeof value === 'string') {
